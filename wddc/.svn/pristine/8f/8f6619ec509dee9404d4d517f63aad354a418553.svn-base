...
<div class="main-container" id="main-container">
						...
						<div class="page-header">
							<h1>
								...
								<span style="float: right;padding-right: 10px;height="41px">
	                              <button type="button" class="btn btn-warning" style="padding:0px" onclick="window.location.href='${ctx}/wdac/dataCheck/toCheckStep1'">添加任务</button>
	                            </span>
							</h1>
						</div><!-- /.page-header -->
										...
										<table id="dynamic-table" class="table table-striped table-bordered table-hover">
											...
											<c:forEach items="${obj.info}" var="info" varStatus="infos">
												<tr>
													...
													<td><a href="#" onclick="window.location.href='${ctx }/wdac/dataCheck/toCheckStep3?id=${info.id}'">${info.taskName}</a></td>
													<td id="checkRule_${infos.index}">${info.taskRules }</td>
													<td >${(info.jobId != null && info.jobId != '') ? '是':'否' }</td>
													<td ><fmt:formatDate value="${info.lastUpdate }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
													<td >
	...
	<script>
		...
		function toAssociate(taskNumber){
			window.location.href="${ctx}/wdac/dataCheck/toJobGl"
		}
		
		function delect(id){
			if (confirm("确定删除吗？")) {  
				$.post("${ctx}/wdac/dataCheck/delectTask",{id:id},function(){
					location.reload();
				});
			}
		}
