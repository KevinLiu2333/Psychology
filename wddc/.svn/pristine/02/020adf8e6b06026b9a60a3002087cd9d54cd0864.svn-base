...
public class ExcelUtil {
	...
	public static ArrayList<ArrayList<Object>> readExcel(File file,int rowNum){
		...
		if(file.getName().endsWith("xlsx")){
			//处理ecxel2007
			return readExcel2007(file,rowNum-1);
		}else{
			//处理ecxel2003
			return readExcel2003(file,rowNum-1);
		}
	}
	/*
				...
				for( int j = row.getFirstCellNum() ; j <= row.getLastCellNum() ;j++){
					...
					switch(cell.getCellType()){
		                ...
		                case XSSFCell.CELL_TYPE_NUMERIC:  
		                    ...
		                    System.out.println(i+1 + "行" + j  
		                            + " 列 is Number type ; DateFormt:"  
		                            + value.toString()); 
		                    break;  
		                case XSSFCell.CELL_TYPE_BOOLEAN:  
		                    System.out.println(i+1 + "行" + j + " 列 is Boolean type");  
		                    value = Boolean.valueOf(cell.getBooleanCellValue());
		                    break;  
		                case XSSFCell.CELL_TYPE_BLANK:  
		                    System.out.println(i+1 + "行" + j + " 列 is Blank type");  
		                    value = "";  
		                    break;  
		                default:  
		                    System.out.println(i+1 + "行" + j + " 列 is default type");  
		                    value = cell.toString();  
					}// end switch
					colList.add(value);
				}//end for j
