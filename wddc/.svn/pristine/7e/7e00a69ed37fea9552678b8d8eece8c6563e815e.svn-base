...
<div class='container'>
    <h3 id="disable-responsive2" class="page-header">组合统计数据服务发布</h3>
    <form method="post" id="publishForm" action="${ctx}/fw/saveJiekou3">
        <input type="hidden" name="fwInfo.fwType" value="组合统计数据服务"/>
        <input type="hidden" name="fwInfo.fwTypeCode" value="Jiekou3">
   ...
   <jsp:include page="base.jsp"/>
    ...
    <div class="col-md-12 form-group">
      ...
      <div class="col-md-12 form-group">
        分组条件设置：
        <select multiple="" id="groupItems" name="groupItems"  class="select2" data-placeholder="请选择分组条件...">
            <option value="">&nbsp;</option>
        </select>
    </div>
     <div class="col-md-12 form-group">
    返回结果设置：
         <select name="resultItems"  id="resultItems" style="width:200px;height: 30px">
            <option value="">----</option>
        </select>
    <select name="resultOps"  style="width:80px;height: 30px">
            <option value="">----</option>
    ...
    function changeZy(){
        $.ajax({
            ...
            success:function(data){
                ...
                $("#resultItems").html("<option value=''>--------</option>");
                for(var i=0;i<data.result.length;i++){
                    $("#zyItems").append("<option value='"+data.result[i].colCfgId+"'>"+data.result[i].colName+"</option>");
                    $("#groupItems").append("<option value='"+data.result[i].colCfgId+"'>"+data.result[i].colName+"</option>");
                    $("#whereItems").append("<option value='"+data.result[i].colCfgId+"'>"+data.result[i].colName+"</option>");
                    $("#resultItems").append("<option value='"+data.result[i].colCfgId+"'>"+data.result[i].colName+"</option>");
                }
