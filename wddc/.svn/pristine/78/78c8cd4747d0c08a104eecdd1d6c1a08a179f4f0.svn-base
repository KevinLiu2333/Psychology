...
//（2）修改dream.form.css，指定弹出窗口大小
//（3）在myplugins数组中增加插件[插件名称,插件标题,插件类型]，插件名称规则和html文件规则雷同：dream-[控件类型]，
//     插件标题是弹出框标题，插件类型是在ueditor中可自动识别的类型见line81
//=============================================================================
...
(function () {
	var myplugins = [
		...
		['dream-radio', '单选框', 'input'],
		['dream-checkbox', '复选框', 'input'],
		//复合组件
        ...
        (function (p) {			
            ...
            UE.plugins[p[0]] = function () {
            	...
            	var popup = new UE.ui.Popup( {
            		...
            		content: '<nobr>' + p[1] + ': <span onclick=$$._edittext() class="edui-clickable">编辑</span></nobr>',
...
/*
	//在toolbar上注册按钮
	UE.registerUI('dream-form',function(editor,uiName){
...
*/
	//创建插件
	UE.plugins['dream-form'] = function () {
		var me = this;
			
		me.commands['dream-form'] = {
			execCommand:function () {
				var dialog = new UE.ui.Dialog({
					iframeUrl:this.options.UEDITOR_HOME_URL + 'dreamform/dream-form.html',
					editor:this,
					name:'dream-form',
					title: '表单设计器',
    ...
    UE.registerUI('wordOrExcelHtmlCleaner', function(editor, uiName) {                
        ...
        var btn = new UE.ui.Button({
            ...
            cssRules: 'background-position: -41px -0px;',
            //点击时执行的命令
            onclick: function() {
