$.fn.fishBone = function(data) {
    ...
    function creataFishBone(data) {
        ...
        $(data).each(function(index) {
            ...
            var color = getColor(index);//#F89782
            var lineFirstY = getLineFirstY(index);//0px
            
            var titleLeftY = getTitleLeftY(index);//0px
            var titleCenterY = getTitleCenterY(index);//-589px
            var titleRightY = getTitleRightY(index);//-1200px
            ...
            if(itemclass=='top'){
                $.each(this, function(name, value) {
                    if (name == '委办局') {
                        var li = $("<li class='line-first'>" +name+": "+value + "</li>")
                ...
                $.each(this, function(name, value) {
                    if (name == '运行状态') {
                        ...
                        var titleCenter =  $("<span class='title-center'>"+name+" : "+value+"</span>").css('background-position-y',titleCenterY);
				...
				$.each(this, function(name, value) {
                if (name == '') {
                    var li = $("<li>" + "<br\>" + "</li>").css("border-left","1px solid "+color);
                    li.appendTo(ul);
                }
            });
            }
