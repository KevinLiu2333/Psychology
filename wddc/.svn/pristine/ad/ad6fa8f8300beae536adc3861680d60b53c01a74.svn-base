...
<body>
	...
	<div class="container">
       	<div class="col-md-12">
       		<h2 class="page-header" style="margin-top: 5px">缓存数据项配置
...
//新增
function add(){
	window.location.href="${ctx}/suite/data/df/toEdit";
}
//编辑
function edit(id){
	window.location.href="${ctx}/suite/data/df/toEdit?id="+id;
}
//查看
function view(id){
	window.open( "${ctx}/suite/data/df/view?id="+id);
	location.reload();
}
//删除
function del(id){
	$.post("${ctx}/suite/data/df/isUsing",{'id':id},function(data){
		if(data == '0'){
			if(window.confirm('你确定要删除该统计信息吗？')){
				$.post("${ctx}/suite/data/df/delete?id="+id,{},function(){
					location.reload();
				});
			}
		}else{
