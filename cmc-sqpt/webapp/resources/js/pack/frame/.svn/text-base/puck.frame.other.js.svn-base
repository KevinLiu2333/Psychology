...
closeTab($(this).attr('index'));});tabsContainer.delegate('div > a.close','click',function(){closeTab($(this).attr('index'));});};function addTab(param){$("#framebg").length>0&&$("#framebg").hide();if(!param.id)return;if(exists(param.id))
