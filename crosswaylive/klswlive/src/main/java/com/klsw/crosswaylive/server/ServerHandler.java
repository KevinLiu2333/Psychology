...
import com.klsw.crosswaylive.server.constants.Services;
...
public class ServerHandler extends ChannelHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        try {
            ...
            //请求参数验证
            if (message == null || !Services.LIST.contains(message.getService()) || !Types.LIST.contains(message.getData().getType()) || !tcpService.verify(ctx, message)) {
                tcpService.res(ctx, "verify", "E", "fail");
                return;
            }
            tcpService.execute(ctx, message);
        } catch (Exception e) {
            tcpService.res(ctx, "verify", "E", "fail");
        }
    }
