	...
	public View match(ActionContext context) {
		if(context.getMethod() != null){
			...
			if(LogManager.logConfigMap.containsKey(classPath+"."+methodName)){
				...
				LogService logService = context.getIoc().get(LogService.class);
				logService.insertLogAndSms(opLogConfig.getCatalog(), opLogConfig.getLogType(), opLogConfig.getIsSms(), context.getRequest());
			}
		}
