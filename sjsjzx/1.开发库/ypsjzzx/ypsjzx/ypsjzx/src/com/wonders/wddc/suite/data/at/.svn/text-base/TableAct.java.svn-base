...
public class TableAct extends Service {
	...
	public void toSaveTable(
			...
			@Param("::tableConfig.") TableConfigBo tableConfig,
...
//		return UI.ajaxDone(StatusCode.OK);
	...
	@At
	@Ok("redirect:/suite/config/table/toTableList")
	public void createDataByTableName(@Param("dataSouceId")String dataSourceId,@Param("viewName")String viewName,@Param("tableCatalog")String tableCatalog,String tableComment){
	...
	public  String updateAll(
		...
		for(ColConfigBo colConfig:oldColConfigList){
			if(newSet != null){
				if(!newSet.contains(colConfig.getColName())){
					toDelList.add(colConfig);
				}else{
					colConfig.setColType(newColMap.get(colConfig.getColName()).getColType());
					String colComment = newColMap.get(colConfig.getColName()).getColComment();
					if(colComment != null && colComment != ""){
						colConfig.setColComment(colComment);
					}
					toUpdateList.add(colConfig);
				}
			}
		}
		...
		for(ColConfigBo colConfig:toDelList){
			dao().delete(colConfig);
		}
		
		for(ColConfigBo colConfig:toUpdateList){
			dao().update(colConfig);
		}
		
		for(ColConfigBo colConfig:toInsertList){
