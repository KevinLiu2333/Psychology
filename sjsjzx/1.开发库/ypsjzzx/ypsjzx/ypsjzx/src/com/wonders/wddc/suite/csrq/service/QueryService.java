...
import com.wonders.wddc.suite.csrq.constants.DetailQueryConstants;
...
import com.wonders.wddc.tiles.tools.NutzSqlUtil;
...
public class QueryService{
	 ...
	 */
	public List<TableConfigBo> queryCatalogList(){
		...
		try{
			...
			sql.setCallback(new SqlCallback() {
				...
				public Object invoke(Connection con, ResultSet rs, Sql sql)
						throws SQLException {
					List<TableConfigBo> resultList = new ArrayList<TableConfigBo>();
					while(rs.next()){
			...
			dao.execute(sql);
			return sql.getList(TableConfigBo.class);
		}catch (Exception e) {
	 ...
	 * @return
	 */
	public QueryConfigBo queryQuerySave(String accountBookId){
		List<QueryConfigBo> querySaveList = dao.query(QueryConfigBo.class, Cnd.where("userId", "=", accountBookId));
	 ...
	 * @return QueryTheme对象列表
	 */
	public List<TableConfigBo> queryThemeListByCatalog(String catalog){
		Criteria cri = Cnd.cri();
		cri.where().andEquals("validity","1");
		cri.getOrderBy().desc("themeId");
		if(Strings.isBlank(catalog)){
			return dao.query(TableConfigBo.class, cri);
		}
	 ...
	 * @return 
	 */
	public List<QueryConfigBo> querySaveByName(String saveName){
		return dao.query(QueryConfigBo.class, Cnd.where("name", "like", "%"+saveName+"%"));
	}
	/**
	 * 根据分类、名称获取查询定义
	 * @param saveName
	 * @return
	 */
	public List<QueryConfigBo> querySave(String saveName,String [] themeIds){
		Criteria cri = Cnd.cri();
		...
		return dao.query(QueryConfigBo.class, cri);
	}
	/**
	 * 获取全部主题列表
	 * @returnQueryTheme 对象列表
	 */
	public List<TableConfigBo> queryAllThemeList(){
		//返回全部列表
		return dao.query(TableConfigBo.class, null);
	}
	 ...
	 * @return QueryTheme对象列表
	 */
	public List<TableConfigBo> queryThemeList(String [] themeIds){
		Criteria cri = Cnd.cri();
		cri.where().andIn("themeId", themeIds);
		return dao.query(TableConfigBo.class, cri);
	}
	 ...
	 * @return
	 */
	public List<ColumnConfigBo> queryPkByThemeId(Integer themeId){
		Criteria cri = Cnd.cri();
		cri.where().and("isPk", "=", "1").and("themeId", "=", themeId);
		return dao.query(ColumnConfigBo.class, cri);
	}
	 ...
	 * @return 
	 */
	public List<QueryConfigBo> querySaveListByTheme(String [] themeIds){
		Criteria cri = Cnd.cri();
		cri.where().andIn("themeId", themeIds);
		cri.getOrderBy().desc("queryCount");
		return dao.query(QueryConfigBo.class, cri);
	}
	public QueryConfigBo querySaveById(String saveId){
		return dao.fetch(QueryConfigBo.class, saveId);
	}
	public List<QueryConfigBo> queryAllQueryConfig(){
		return dao.query(QueryConfigBo.class, null);
	}
	 ...
	 * @return 
	 */
	public List<QueryConfigBo> querySaveByRefId(String [] refIds){
		Criteria cri = Cnd.cri();
	 ...
	 */
	public List<ColumnConfigBo> queryColumnListByColId(String [] colIds){
		List<ColumnConfigBo> result = new ArrayList<ColumnConfigBo>();
		for (String colId : colIds){
			ColumnConfigBo columnConfig = dao.fetch(ColumnConfigBo.class,Integer.parseInt(colId));
	...
	public ColumnConfigBo queryColumnByColId(Integer colId){
		return dao.fetch(ColumnConfigBo.class,colId);
	}
	 ...
	 * @return
	 */
	public List<QueryConfigBo> querySaveList(){
		Criteria cri = Cnd.cri();
		cri.getOrderBy().desc("queryCount");
		return dao.query(QueryConfigBo.class, cri) ;
	}
	 ...
	 * @return
	 */
	public TableConfigBo getQueryThemeById(Integer themeId){
		return dao.fetch(TableConfigBo.class, themeId);
	}
	...
	public List<ColumnConfigBo> queryColumnList(Integer themeId){
	 ...
	 * @param query 定义对象
	 */
	public QueryConfigBo addQuerySave(QueryConfigBo query){
		QueryConfigBo querySave = dao.insert(query);
		return querySave;
	}
	 ...
	 * @param query 定义对象
	 */
	public void updateQuerySave(QueryConfigBo query){
		dao.update(query);	
	...
	public void removeQuerySave(String saveId){
		dao.delete(QueryConfigBo.class, saveId);
	}
	...
	public void addQueryCount(String saveId,int queryCount){
		dao.update(QueryConfigBo.class, Chain.make("queryCount", queryCount), Cnd.where("saveId", "=", saveId));
		dao.update(TableConfigBo.class, Chain.make("queryCount", queryCount), Cnd.where("saveId", "=", saveId));
	}
	...
	public TableConfigBo addQueryTheme(TableConfigBo tableConfig) {
	...
	public FormConfigBo addFormConfig(FormConfigBo formConfig){
		return dao.insert(formConfig);

	}
	
	public FormConfigBo queryFormConfig(String id){
		return dao.fetch(FormConfigBo.class, id);
	}
	/**
	 * 查询当前数据库用户的所有表和视图
	 * @return
	...
	public void saveQueryColumns(TableConfigBo tableConfig) {
		...
		int cursor = 1;
		List<ColumnConfigBo> queryColumnList = new ArrayList<ColumnConfigBo>();
		...
		for (Map<String,String> columnMap : columnMapList) {
			...
			ColumnConfigBo queryColumn = new ColumnConfigBo();
