...
<form id="form1" action="${ctx }/suite/data/df/toSavaInfo" method="post">
	...
	<div class="row col-md-offset-1">
		...
		<div class="col-md-6 form-group">
		     	<div  style="float: left;width: 150px;"> 文件名称：</div>
		     	<input id="filename" style="width: 250px;"  name="info.filename" value="${obj.info.filename }">
		</div>
		<div class="col-md-6 form-group">
		     	<div  style="float: left;width: 150px;"> 文件路径：</div>
		     	<input id="filepath" type="text" style="width: 250px;"  name="info.filepath"  value="${obj.info.filepath }" />
		</div>
	</div>
...
function addCustomTb(){
	...
	$(cloneTr).find("input").each(function(){
		...
		$(this).attr("name",thisName.replace("info.customList[0]","info.customList["+ customIndex + "]"));
