...
<div class="pageHeader">
	<form id="queryForm" onsubmit="return navTabSearch(this);" action="query/statChart/toTheme" method="post">
		<div class="searchBar">
...
<div class="pageFormContent" layoutH="55">
	...
	<div class="panelBar">
		<ul class="toolBar">
			...
			<li><a class="add" href="query/statChart/toDefine?themeId=${queryTheme.themeId}" target="navTab" rel="queryDefine" title="添加图形统计"><span>添加</span></a></li>
		</ul>
	</div>
	<!-- （2）显示列表 -->
	<table  width="100%">
			...
			<c:if test="${save.themeId == queryTheme.themeId}">
				<div style="float: left;margin: 5px;width: 30%">
						<c:if test="${not empty obj.saveName && fn:contains(save.name,obj.saveName)}">
							<a href="query/statChart/toDefine?saveId=${save.saveId}" title="修改图形统计" target="navTab" rel="queryDefine"><span style="background-color:#ffff80;color:#ee6600">${save.name }${saveName}</span></a>
						</c:if>
						<c:if test="${ empty obj.saveName || !fn:contains(save.name,obj.saveName)}">
							<a href="query/statChart/toDefine?saveId=${save.saveId}" title="修改图形统计" target="navTab" rel="queryDefine"><span >${save.name }</span></a>
						</c:if>
						<a href="query/statChart/showChart?saveId=${save.saveId}"  target="navTab" rel="showQuery" title="预览图形统计">(预览)</a>
				</div>
			</c:if>
		</c:forEach>
		</td></tr>
