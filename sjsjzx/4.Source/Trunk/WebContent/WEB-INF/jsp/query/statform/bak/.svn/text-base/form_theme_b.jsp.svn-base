...
<%@ page import="com.wonders.tiles.query.statform.entity.QueryTheme"%>
...
function showQuery(saveId){
    $("#queryForm").attr("action","query/statForm/showForm?saveId="+saveId);
	$("#queryForm").submit();
}
//调整主题分类
function toCatalogTheme(filter){
    $("#filterType").val(filter);
	$("#queryForm").attr("action","query/statForm/toTheme");
	$("#queryForm").submit();
}
//调整主题分类
//修改定义
function editDefine(saveId,catalog){
	$("#queryForm").attr("action","query/statForm/toDefine?saveId="+saveId);
	$("#queryForm").submit();
}
//新增定义
function addDefine(themeId,catalog){
    $("#queryForm").attr("action","query/statForm/toDefine?themeId="+themeId);
	$("#queryForm").submit();
}
</script>
...
<tr align="right" height="30">
 ...
 <%}}} %></select>
  ...
  <th align="left"  width="70%">表格名称快速搜索：<input type="text"  name="saveName" id="saveName" size="40" value="<%=StringUtils.trimToEmpty(saveName) %>"/>
	   ...
	   int index=0;
	    if(queryListsave!=null&&queryListsave.size()>0){ 
	    	 for(int i=0;i<queryListsave.size();i++){
	    		 FormSave querySave = (FormSave)queryListsave.get(i);
