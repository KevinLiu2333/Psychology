...
<body style="background: url(${ctx}/skins/images/white_bg.gif);">
	...
	<form id="MainForm" action="" method="post">
	    ...
	    <div >
	    		<table width="80%" style="margin-left:10%" align="center" class="table_multilist" >
			           <tr>
	           ...
	           <tr>
					...
					<td width="35%" class="label_2" colspan="2"><wd:dicvalue dicId="1056" dicCode="${obj.resource.shareProperty}"/></td>
			    ...
			    <table width="80%"  id="starlist" class="table_multilist" style="margin-left:15%">
			    	   <tr style="background-color: silver;">
			           		<td class="label_1" width="20%" style="text-align: center;">资源项名</td>
			           		<td class="label_1" width="19%" style="text-align: center;">数据类型</td>
			           		<td class="label_1" width="20%" style="text-align: center;">备注</td>
			           		<td class="label_1" width="17%" style="text-align: center;">共享属性</td>
			           ...
			           <c:forEach items="${obj.fieldDetailsList}" var="fieldDetails" varStatus="row">
			           		<c:if test="${row.index%2 == 0}">
			           			<tr class="bg_white">
				           			<td align="center" class="fontClass">${fieldDetails.dataItemName}</td>
					           		<td align="center" class="fontClass">
					           			<wd:dicvalue dicId="1055" dicCode="${fieldDetails.dataItemType}"/>
					           		</td>
					           		<td align="center" class="fontClass">${fieldDetails.memo}</td>
					           		<td align="center" class="fontClass">
					           			<wd:dicvalue dicId="1056" dicCode="${fieldDetails.shareProperty}"/>
					           		</td>
					           		<td align="center" class="fontClass">
					           			<wd:dicvalue dicId="1057" dicCode="${fieldDetails.openProperty}"/>
					           		</td>
					           		<td align="center" class="fontClass">${fieldDetails.noOpenReason}</td>
				           		</tr>
			           		</c:if>
			           		<c:if test="${row.index%2 != 0}">
				           		<tr class="bg_grey">
	    ...
	    <div style="text-align: center;margin-top: 20px;padding-bottom: 10px;margin-left: 25px;">
	     <c:if test="${obj.resource.resourceId  =='e2f92fd91fc94a82b2703ee022caa5d3'}">
		   <input type="button" id="button1"  class="button" value="数据预览" onclick="querycreateTable1();"/>&nbsp;&nbsp;
		    </c:if>
		  ...
		  <c:if test="${obj.resource.resourceId  !='e2f92fd91fc94a82b2703ee022caa5d3'}">
		   <input type="button" id="button1"  class="button" value="数据预览" onclick="querycreateTable('${obj.resource.tableName}','${obj.resource.resourceId}');"/>&nbsp;&nbsp;
		    ...
		    <c:if test="${obj.user !=null}">
				<input type="button" id="button"  class="button" value="资源共享申请" onclick="applyResource('${obj.resource.resourceId}');"/>&nbsp;&nbsp;
		    </c:if>
				<input type="button" id="button"  class="button" value="返  回" onclick="window.close();"/> 
		</div>
	</form>
...
//数据预览
      function querycreateTable(table,resourceId){
  		window.showModalDialog("${ctx}/zymlgl/queryTablie?table="+table+"&resourceId="+resourceId,self,"dialogWidth=1060px;dialogHeight=600px;resizable:no;status:no;scroll:yes;");
  	}
    //数据预览（示例）
      function querycreateTable1(){
  		window.showModalDialog("${ctx}/zymlgl/renkou",self,"dialogWidth=960px;dialogHeight=600px;resizable:no;status:no;scroll:yes;");
  	}
      //资源申请
