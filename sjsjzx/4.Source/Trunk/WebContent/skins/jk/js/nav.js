$(document).ready(function(){
	var nav = $(".nav"),
		len = $(".nav ul li").length,
		nScreen = $(".nav .navScreen"),
		ul = $(".nav ul"),
		left = $(".nav .navLeft"),
		right = $(".nav .navRight"),
		stepR = "-=" + ($(".nav ul li").width() + 2)+ "px",
		stepL = "+=" + ($(".nav ul li").width() + 2)+ "px",
		liWidth = $(".nav ul li").width() + 2;
	ul.css("width", len*liWidth);
	if(ul.width() > nScreen.width()){right.show();}
	right.click(function(){
		ul.stop(false,true).animate({left: stepR}, "slow");
		//2015-7-3 jiaojun 修改
		if((-ul.position().left) + liWidth >= (ul.width() - nScreen.width())){right.hide();left.show();}
		else if(ul.position().left - liWidth){left.show();}
	})
	left.click(function(){
		ul.stop(false,true).animate({left: stepL}, "slow");
		if((ul.position().left + liWidth) >= 0){
			left.hide();
			if(ul.width() > nScreen.width()){right.show();}
		}
		else if((ul.position().left + liWidth) < (ul.width() - nScreen.width())){right.show();}
	})
})
function dc(){
