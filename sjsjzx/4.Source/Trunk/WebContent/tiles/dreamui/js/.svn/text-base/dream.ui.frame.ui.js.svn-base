function initEnv() {
	...
	var url = $('li[tabid=main]').attr('url');
	
	var $fun = $('<a href="' +url+ '" target="navTab" rel="main" style="display:none">main</a>');
	$('body').append($fun);
...
function initLayout(){
	var iContentW = $(window).width() - (DWZ.ui.sbar ? $("#sidebar").width() + 10 : 34);
	var iContentH = $(window).height() - $("#header").height();

	$("#container").width(iContentW);
	$("#container .tabsPageContent").height(iContentH - 15).find("[layoutH]").layoutH();
