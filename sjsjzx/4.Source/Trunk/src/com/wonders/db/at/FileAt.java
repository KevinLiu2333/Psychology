...
public class FileAt {
	...
	private Xdb_files upload(TempFile tempFile,String fileType)
	{
	...
	public Object uploadFile(TempFile tempFile,HttpServletRequest req,HttpSession session,String fileType) throws IOException{
		Xdb_files newFile =upload(tempFile,fileType);
	...
	public Object uploadApiFile(TempFile tempFile,HttpServletRequest req,HttpSession session) throws IOException{
		Xdb_files newFile =upload(tempFile,null);
		Map<String,Object> result= new HashMap<String,Object>();
		result.put("file", newFile);
		return result;
	}
	...
	@At
	@Filters
	public void filedel(String fid, HttpServletResponse response) {
		if (!Strings.isEmpty(fid)) {
			Xdb_files resource = dao.fetch(Xdb_files.class, Cnd.where("FID", "=",fid));
			if (resource != null) {
				 List<Xdb_files> resourceDetailsList =
				 dao.query(Xdb_files.class, Cnd.where("FID", "=",fid));
				 if(resourceDetailsList.size() >0){
				 for(Xdb_files resourceDetails : resourceDetailsList){
