...
public class QueryAt extends EntityService<QueryConfig> {
	...
	@At
    public View toResult(HttpSession session,HttpServletRequest req,String saveId,String queryFlag,String exportFlag,String viewFlag,String onekey, String fromMenu) throws Exception {
...
//        	queryRenderer = new HashMap<String,Object>();
        ...
        if (querySave != null && StringUtils.isBlank(queryFlag)) {// 查询结果
			...
			//查人口库时用*去遮住身份证号和姓名
			if("1460623452768".equals(saveId)){
				hiddenPeoplesInfo(rsList);
			}
			//隐藏房屋基本信息
			if("1461291529301".equals(saveId)){
				hiddenHouseInfo(rsList);
			}
...
//                	String b = DicDataUtils.getInstance().getDicData(DetailQueryConstants.DIC_QUERY_COL, tempColIds[i]);
     ...
     * @param list
     */
    private void hiddenPeoplesInfo(List list){
    	for(Object obj:list){
    		...
    		if(csrq != null){
    			map.put("CSRQ", csrq.replace(csrq, csrq.subSequence(0, 4)+"****"));
    		}
     ...
     * @param list
     */
    public void hiddenHouseInfo(List list){
    	for(Object obj:list){
    		Map<String, String> map=(Map<String, String>) obj;
    		String fzzjhm = map.get("FZZJHM");//房主证件号码
    		if(!Strings.isEmpty(fzzjhm)){
    			if(fzzjhm.length()==18){
    				map.put("FZZJHM", fzzjhm.substring(0,6)+"********"+fzzjhm.substring(14,18));
    			}else {
					map.put("FZZJHM", Strings.dup('*', fzzjhm.length()/2)+fzzjhm.substring(fzzjhm.length()/2,fzzjhm.length()));
				}
    		}
    		String fzlxdh = map.get("FZLXDH");
    		if(!Strings.isEmpty(fzlxdh)){
    			map.put("FZLXDH", fzlxdh.substring(0, fzlxdh.length()-3)+"***");
    		}
    		//房主姓名
    		String fzxm = map.get("FZXM");
    		if(!Strings.isEmpty(fzxm)){
    			if(fzxm.length() == 3){
    				map.put("FZXM", fzxm.replace(fzxm.substring(1, 2), "*"));
    			}else if(fzxm.length() == 2){
    				map.put("FZXM", fzxm.replace(fzxm.substring(1), "*"));
    			}else{
    				map.put("FZXM", fzxm.replace(fzxm.substring(1, 2), "**"));
    			}
    		}
