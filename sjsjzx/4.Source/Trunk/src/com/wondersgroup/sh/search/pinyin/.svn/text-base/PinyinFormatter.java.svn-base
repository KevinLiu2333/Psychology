...
import com.wondersgroup.sh.search.pinyin.format.BadHanyuPinyinOutputFormatCombination;
import com.wondersgroup.sh.search.pinyin.format.HanyuPinyinCaseType;
import com.wondersgroup.sh.search.pinyin.format.HanyuPinyinOutputFormat;
import com.wondersgroup.sh.search.pinyin.format.HanyuPinyinToneType;
import com.wondersgroup.sh.search.pinyin.format.HanyuPinyinVCharType;

public class PinyinFormatter {
	static final String allUnmarkedVowelStr = "aeiouv";
	static final String allMarkedVowelStr = "āáǎàaēé��èeīí��ìiōó��òoūú��ùuǖǘǚǜü";
   ...
   */
	...
	static String convertToneMark2Unmarked(final String pinyinStr) {
		String retStr = "";
		char[] chars = pinyinStr.toCharArray();
		for (int i = 0; i < chars.length; i++) {
			char c = chars[i];
			int index = allMarkedVowelStr.indexOf(c);
			if( index >= 0 ) {
				c = allUnmarkedVowelStr.charAt(index / 5);
			}
			retStr += c;
		}
		return retStr;
		...
		if (lowerCasePinyinStr.matches("[a-z]*[1-5]?")) {
			...
			if (lowerCasePinyinStr.matches("[a-z]*[1-5]")) {
				...
				if ((defautlCharValue != unmarkedVowel) && (defautlIndexValue != indexOfUnmarkedVowel)) {
					...
					resultBuffer.append(lowerCasePinyinStr.substring(indexOfUnmarkedVowel + 1, lowerCasePinyinStr.length() - 1)
							.replaceAll("v", "ü"));
	...
	public static void main(String[] args) {
		System.out.println(convertToneMark2Unmarked("máodòu"));
		System.out.println(convertToneMark2Unmarked("zhūlǚ"));
	}
}
