...
import com.wondersgroup.sh.search.ChineseTokenizer;
...
public class BayesClassifier {
	 ...
	 * 默认的构造器，初始化训练集
	 */
	public BayesClassifier(String trainnigDataPath, Analyzer analyzer) {
		this.trainnigDataPath = trainnigDataPath;
		this.analyzer = analyzer;
		tdm = new TrainingDataManager(trainnigDataPath);
	}
	...
	float calcProd(String[] X, String Cj) {
		...
		for (int i = 0; i < X.length; i++) {
			...
			ret *= new ClassConditionalProbability(tdm).calculatePxc(Xi, Cj) * zoomFactor;
		}
		// 再乘以先验概率
		ret *= new PriorProbability(tdm).calculatePc(Cj);
		return ret;
	}
	...
	public String classify(String text) {
		String[] terms = new ChineseTokenizer(this.analyzer).tokenize(text);
		...
		//返回概率最大的分类
		return ((ClassifyResult) crs.get(0)).classification;
	}
	...
	public static void main(String[] args) {
		...
		BayesClassifier classifier = new BayesClassifier("C:\\Downloads\\train", new MMAnalyzer());//构造Bayes分类器
