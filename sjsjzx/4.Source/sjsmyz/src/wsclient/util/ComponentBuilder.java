...
public class ComponentBuilder {
	...
	public ServiceInfo buildserviceinformation(ServiceInfo serviceinfo) throws Exception {
		...
		log.debug("WSDL的Schema已经成功构建完,总共有多少个Schema定义:" + wsdlTypes.size());

		Map services = def.getServices();
		if (services != null) {
			Iterator svcIter = services.values().iterator();
			populateComponent(serviceinfo, (Service) svcIter.next());
			log.debug("***恭喜您!系统的Web服务对象:ServiceInfo已经成功构建***");
			log.debug("");
		}
		return serviceinfo;
	}
	...
	private Schema createschemafromtype(org.w3c.dom.Element schemaElement, Definition wsdlDefinition) {
		log.debug("现在的Schema还是一个Dom型的<xsd:schema>元素,属性还不够完全,必须构建命名空间等属性");
		log.debug("使用JDom,先把Dom型的<xsd:schema>元素转换成JDom型...");
		log.debug("开始构建...");
		...
		if (namespaces != null && !namespaces.isEmpty()) {
			log.debug("WSDL文档Definition的所有命名空间为:");
	...
	protected Vector createSchemaFromTypes(Definition wsdlDefinition) {
		log.debug("*****************");
		log.debug("进入createSchemaFromTypes()方法");
		log.debug("开始从Types中产生Schema,Definition下types元素的Schema元素个数可以多个,传进来的参数是一个Definition对象:");
	...
	private ServiceInfo populateComponent(ServiceInfo component, Service service) {
		log.debug("***************************");
		log.debug("");
		log.debug("***开始构建系统的Web服务对象:ServiceInfo***");
		QName qName = service.getQName();
		String namespace = qName.getNamespaceURI();
		log.debug("namespace为:" + namespace);
		component.setTargetnamespace(namespace);
		String name = qName.getLocalPart();
		log.debug("name为:" + name);
	...
	private List buildOperations(Binding binding) {
		log.debug("进入buildOperations(Binding binding)方法,构建服务所有的操作对象List buildOperations");
	...
	private OperationInfo buildOperation(OperationInfo operationInfo, BindingOperation bindingOper) {
		log.debug("从一个BindingOperation对象(<wsdl:operation>)构建OperationInfo对象");
		...
		log.debug("开始转移到了<wsdl:portTyp>结点下的<wsdl:input>");
		if (inDef != null) {
			...
			if (inMsg != null) {
				...
				log.debug("***操作:" + operationInfo.getTargetMethodName() + "的所有输入参数已经构建完毕***");
				log.debug("");
		...
		if (outDef != null) {
			...
			if (outMsg != null) {
				...
				log.debug("***操作:" + operationInfo.getTargetMethodName() + "的所有输出参数已经构建完毕***");
				log.debug("");
	...
	private void getParameterFromMessage(OperationInfo operationInfo, Message msg, int manner) {
		...
		log.debug("");
		log.debug("***开始构建" + operationInfo.getTargetMethodName() + "操作的所有消息" + tip + "参数***");
		...
		if (group != null) {
			...
			while (groupEnum.hasMoreElements()) {
				...
				if (item.getStructureType() == Structure.ELEMENT) {
					...
					log.debug("复杂数据类型的子元素是一个Element:" + elementDecl.getReferenceId());
					XMLType xmlType = elementDecl.getType();
					if (xmlType != null && xmlType.isComplexType()) {
						log.debug("***" + elementDecl.getReferenceId() + "元素是一个复杂类型,进入递归调用****");
						buildComplexParameter((ComplexType) xmlType, operationInfo, manner);

					} else {

						log.debug("现在开始处理简单数据类型");
						ParameterInfo parameter = new ParameterInfo();
						parameter.setName(elementDecl.getName());
						log.debug("参数名为:" + elementDecl.getName());
						parameter.setKind(elementDecl.getType().getName());
						log.debug("参数类型为:" + elementDecl.getType().getName());
						if (manner == 1) {
							operationInfo.addInparameter(parameter);
						} else {
							operationInfo.addOutparameter(parameter);
	...
	protected XMLType getXMLType(Part part, Schema wsdlType, OperationInfo operationInfo) {
		...
		if (part.getElementName() != null) {
			...
			log.debug("part引用的类型名为:" + elemName);
