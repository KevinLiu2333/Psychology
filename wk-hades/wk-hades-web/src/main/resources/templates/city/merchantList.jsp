...
var dtGridColumns = [{
    ...
    resolution : function(value, record, column, grid, dataNo, columnNo) {
    	var selectHtml='<button class="btn btn-sm btn-default" onclick="select('+record.id+')">选择</button>';
        return '<button class="btn btn-sm btn-default" onclick="editModel('+record.id+',\'/topic/editConUI\',\'查看\',\'460px\',\'790px\')">查看</button>&nbsp;'+selectHtml;
    }
}];
...
function select(merchantId) {
	layer.confirm('确定要选择该门店吗?', {icon: 3, title:'提示'}, function(index){
        ajaxPost('/city/addCityCategoryMerchant',{cityCategoryId:22, merchantId:merchantId});
    });
}
</script>