...
<div class="box box-info">
    ...
    <!-- /.box-header -->
    <!-- form start -->
    <form id="topicForm" name="topicForm" class="form-horizontal" role="form" method="post"
          data-fv-message="This value is not valid"
          data-fv-feedbackicons-valid="glyphicon glyphicon-ok"
          data-fv-feedbackicons-invalid="glyphicon glyphicon-remove"
          data-fv-feedbackicons-validating="glyphicon glyphicon-refresh">
          
        <input type="hidden" name="id" value="${cityTopic.id}" id="id" />
        <div class="box-body">
            <div class="form-group">
                <label class="control-label col-sm-2 no-padding-right" for="categoryName">专题名称</label>
                <div class="col-sm-10">
                    <div class="clearfix">
                        <input class="form-control" name="name" value="${cityTopic.name}" id="name" type="text" placeholder="专题名称..."
                               data-fv-notempty="true"
                               data-fv-message="专题名称不能为空"/>
                    </div>
                </div>
            </div>
            <div class="form-group">
                <label class="control-label col-sm-2 no-padding-right" for="pic_file_upload">专题图片</label>
                <div class="col-sm-2">
                     <input type="hidden" id="pic" name='pic' value="${cityTopic.pic}"/>
                     <div id="pic_imgs"><img src="${cityTopic.pic}" style="width:50px;height:50px;" /></div><br/>
                     <div id="pic_queue"></div>
                     <input id="pic_file_upload" name="pic_file_upload" class="form-control" type="file"  multiple="true">
                </div>
            </div>
            <div class="form-group">
                <label class="control-label col-sm-2 no-padding-right" for="pic_file_upload">所属城市</label>
                <div class="col-sm-2">
                     <select id="cityId" name="cityId">
                     	<option value="">==请选择==</option>
	                     <c:forEach items="${cityList}" var="city">
							<option value="${city.id}"<c:if test="${cityTopic.cityId eq city.id }">selected=selected</c:if>>${city.name}</option>                   
	                     </c:forEach>
                     </select>
                </div>
            </div>
        ...
        <div class="box-footer">
            <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                    <button id="btnAdd" type="button" onclick="commit('topicForm','/cityTopic/save');" class="btn btn-success btn-sm">
