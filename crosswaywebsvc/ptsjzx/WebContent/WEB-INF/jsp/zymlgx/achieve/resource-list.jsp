...
<!-- 1、查询条件  -->
<div align="center">
	<form id="queryForm" action="${ctx}/zymlgx/toGetResource" method="post">
...
<div align="center">
	...
	<table class="table_list" style="width:96%;height:96%">
		...
		<tr align="center">
			...
			<td>${resource.checkedPerson}</td>
			<td>${resource.applyTopic}</td>
			<td><wd:dicvalue dicId="<%=Constants.DIC_DEPT_NAME %>" dicCode="${resource.resourceProvider}"/></td>
			<td>
				<c:if test="${resource.status eq '4'}">
				申请成功
				</c:if>
			</td>
			<td>
				<c:if test="${resource.status eq '4'}">
				<a href="javascript:download('${resource.applyId}')">获取</a>
				</c:if>
			</td>
		</tr>
		</c:forEach>
