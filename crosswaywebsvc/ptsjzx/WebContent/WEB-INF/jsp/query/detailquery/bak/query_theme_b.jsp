...
<%@ page import="com.wonders.tiles.query.detailquery.entity.QueryTheme"%>
...
//定义列表
List queryListsave = (List) objMap.get("saveList");
//主题列表
...
function showQuery(saveId){
	$("#queryForm").attr("action","query/detailQuery/queryResult?saveId="+saveId);
	$("#queryForm").submit();
}
//调整主题分类
function toCatalogTheme(filter){
    $("#filterType").val(filter);
	$("#queryForm").attr("action","query/detailQuery/toTheme");
	$("#queryForm").submit();
}
//调整主题分类
//修改定义
function editDefine(saveId,catalog){
	$("#queryForm").attr("action","query/detailQuery/toDefine?saveId="+saveId);
	$("#queryForm").submit();
}
//新增定义
function addDefine(themeId,catalog){
	$("#queryForm").attr("action","query/detailQuery/toDefine?themeId="+themeId);
	$("#queryForm").submit();
}
</script>

<body>
<form name="queryForm" id="queryForm" action="query/detailQuery/toTheme"  method="post" onsubmit="return navTabSearch(this);">
...
<tr align="right" height="30">
 ...
 <%}}} %></select>
  ...
  <th align="left"  width="70%">查询名称快速搜索：<input type="text"  name="saveName" id="saveName" size="40" value="<%=StringUtils.trimToEmpty(saveName) %>"/>
	   ...
	   int index=0;
	    if(queryListsave!=null&&queryListsave.size()>0){ 
	    	 for(int i=0;i<queryListsave.size();i++){
	    		 QuerySave querySave = (QuerySave)queryListsave.get(i);
