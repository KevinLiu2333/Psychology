...
public class QueryGisAt {
	...
	public View getCropgis(String corpinfoid){
		...
		Sql sql =Sqls.create("select x,y from V_EXT_CORP_INFO where CORP_INFO_ID ='"+corpinfoid+"'");
	...
	public View getPersonGis(String rid){
		...
		if(flag&&!Strings.isEmpty(person.getJzdjwhdm())&&!Strings.isEmpty(person.getJzdjddm())&&person.getJzdjddm().startsWith("07")){
			flag = false;
		...
		if(flag&&!Strings.isEmpty(person.getHjdjwhdm())&&!Strings.isEmpty(person.getHjdjddm())&&person.getHjdjddm().startsWith("07")){
			flag = false;
			Map<String, Object> gis = new HashMap<String, Object>();
			gis.put("x", "0");
			gis.put("y", "0");
