...
<script type="text/javascript">
	...
	function deleteApply(applyid){
		if(confirm("确定删除吗？删除后数据不可恢复!")){
			$.post("${ctx}/zymlgx/deleteApply?applyid="+applyid, 
		            { Action: "post"},
...
<div align="center" style="padding-top:10px;">	
	<table width="96%" height="60" align="center" class="query_search" style="border:1px solid #B6CDF7;">
		<tr>
			...
			<td>申请状态：
				<wd:select id="status" className="selectInput" style="width:120px;height:25px;" name="filter_str_status_eq" dicCode="<%=Constants.STATUS_RESOURCE_APPLY%>" initValue="---请选择---" defaultValue="${filter.filter_str_status_eq}"/>
			</td>
...
<div align="center" style="overflow-y:auto; overflow-x:auto;" layoutH="110">
	...
	<table class="table_list" style="width:96%;height:98%">
		...
		<tr  align="center">
			...
			<td>
			<!-- 10932=待领导审核|10935=待科委审核|10933=审核通过|10934=审核不通过-->
				<c:if test="${resourceApply.status eq '10932'}">
					<a href="#" onclick="editApply('${resourceApply.resourceId}','${resourceApply.applyId}')">修改</a>
					<a href="#" onclick="deleteApply('${resourceApply.applyId}')">删除</a>
				</c:if>
				<a href="#" onclick="viewApply('${resourceApply.applyId}')">查看</a>
				<%-- <c:if test="${resourceApply.status eq '10936'}">
				<a href="${ctx}/zymlgx/seeData?applyId=${resourceApply.applyId}">查看数据</a>
				</c:if> --%>
				<c:if test="${resourceApply.status eq '10934'}">
				<a href="#" onclick="editApply('${resourceApply.resourceId}','${resourceApply.applyId}')">修改</a>
				</c:if>
			</td>
		</tr>
