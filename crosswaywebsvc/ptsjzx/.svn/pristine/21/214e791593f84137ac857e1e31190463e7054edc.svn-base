...
<body>
<p style="text-align:center;font-size:20px;padding-bottom: 15px;padding-top: 10px"><b>详细信息</b></p>

	<table width="80%" class="table_multilist" align="center">
		
		<c:forEach items="${obj.list[0] }" var="col">
			<tr>
			    <td width="40%" class="label_1"  style="text-align:right; height:30px">${col.key }</td>
			    <td width="40%" class="label_2">${col.value }</td>
			</tr>		  		  
		</c:forEach>
	...
	<div style="text-align: center;margin-top: 20px;padding-bottom: 10px">
		<input type="button" id="button"  class="button" value="关闭" onclick="window.close();" /> 
	</div>
</body>
</html>