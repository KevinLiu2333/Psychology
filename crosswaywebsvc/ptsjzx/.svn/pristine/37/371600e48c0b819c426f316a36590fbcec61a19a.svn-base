...
<body>
	...
	<div class="content">
		<ul class="conbox_w600">
			...
			<li class="con_bg02">
				<h1>户籍人口、外来人口比例</h1> <span>
					<dl>
						<dt id="wlbl">65%</dt>
						<dd>
							2016年外来人口<b id="wlrk">110</b>万人
						</dd>
					</dl>
					<dl>
						<dt id="hjbl">35%</dt>
						<dd>
							2016年户籍人口<b id="hjrk">65</b> 万人
						</dd>
					</dl>
					<div class="ring_percentage">
						<div id="hjrkbt" style="width: 200px;height: 200px;margin-left: 40px"></div>
					</div>
			</span>
			</li>
		</ul>
	...
	$.post("${ctx}/dp/piequery?info.sqlcode=20160419170707797",
		...
		function drawhjrk(ec){
			$.post("${ctx}/dp/piequery?info.sqlcode=20160420092658251",
					...
					function(data, textStatus){
				data = eval('('+data+')');
				$('#hjrk').html(Math.round(data[1].value/10000));
				$('#wlrk').html(Math.round(data[0].value/10000));
				$('#hjbl').html(Math.round(data[1].value/(data[0].value+data[1].value)*100)+"%");
				$('#wlbl').html(Math.round(data[0].value/(data[0].value+data[1].value)*100)+"%");
					    ...
					    series : [
					        {
					            ...
					            data:[
					                ...
					                {value:data[0].value, name:'外来人口',itemStyle :{normal :{color:"#FF8464"}}}
					            ]
					        }
					    ]
					};
