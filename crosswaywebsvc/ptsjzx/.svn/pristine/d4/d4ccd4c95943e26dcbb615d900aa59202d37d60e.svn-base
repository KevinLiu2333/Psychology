	...
	<table width="70%" class="table_multilist" align="center" style="color: navy;">
		<tr>
			<td align="left">&nbsp;&nbsp;选择所需申请的资源类型：
				...
				<input type="button" class="minButton" style="width:120px;margin-left: 180px;" onclick="query(1)" value="查  询"/>
			</td>
		</tr>
	</table></br>
		...
		<tr>
			<c:forEach items="${obj.rows}" var="list" varStatus="row">
					<c:if test="${row.count%2!=0}">
						...
						<td align="center">
							<c:if test="${list.shareType eq '0'}">
								<font color="#969696">${list.fieldName}</font>
							</c:if>
							<c:if test="${list.shareType eq '1' || list.shareType eq '2'}">
								${list.fieldName}
							</c:if>
						...
						<td align="center">
							...
							<c:if test="${list.shareType eq '1'}">普遍共享</c:if>
