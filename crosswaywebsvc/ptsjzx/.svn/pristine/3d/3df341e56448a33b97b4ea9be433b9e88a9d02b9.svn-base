...
<body>
		...
		<td valign="top" id="list_search" style="height: 120px;">
			...
			<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width="13%" align="left"><wd:select id="dmlx" name="dmlx" dicCode="1068" className="list_search_select" style="width:94%;" defaultValue="${obj.dmlx }" initValue="请选择代码类型" onchange="changedmlx()"/></td>
					...
					<td width="13%"><wd:select id="frzt" name="frzt" dicCode="1051" className="list_search_select" style="width:94%;" defaultValue="${obj.frzt }" initValue="-------请选择-------"/></td>	
				...
				<tr>
					...
					<td width="13%">
						<wd:select id="cylb1" name="cylb1" dicCode="1063" className="list_search_select" style="width:93%;" onchange="changecylb(1)" defaultValue="${obj.cylb1 }" initValue="-------请选择-------"/>
					</td>
				...
				<tr>
					...
					<td width="13%"><wd:select id="frlx" name="frlx" dicCode="1061" className="list_search_select" style="width:94%;" onchange="changefrlx(1)" defaultValue="${obj.frlx }" initValue="-------请选择-------"/></td>
					<td colspan="2">
					...
					<td><wd:datepicker id="checkedDate1" name="zcrq1"
						...
						className="list_search_input" minDate="1900-01-01" onchange="changedate1()" style="width:90%;"
						 /></td>
					<td height="30" align="right">至：</td>
				...
				<tr>
					<td width="5%" height="30" align="right">经营范围：</td>
					<td width="35%" colspan="3"><input id="jyfw" type="text" class="list_search_input" name="jyfw" value="${obj.jyfw }" style="width:95%;" /></td>
					<td width="5%" height="30" align="right">经营场所：</td>
					<td width="35%" colspan="3"><input id="jycs" type="text" class="list_search_input" name="jycs" value="${obj.jycs }" style="width:96%;" /></td>
				</tr>
			</table>
			<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0">
      			<tr>
	...
	<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0">
			...
			<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0"  class="tablelist">
				...
				<c:forEach items="${obj.list}" var="corp" varStatus="row">
					<tr>
						...
						<td align="center"><c:if test="${fn:length(corp.corpname)<20 }">${corp.corpname}</c:if><c:if test="${fn:length(corp.corpname)>=20 }">${fn:substring(corp.corpname,0,19) }...</c:if></td>
						<td align="center"><c:if test="${corp.organcode!=null&&corp.organcode!='NULL'}">${corp.organcode}</c:if></td>
						<td align="center"><c:if test="${corp.personname!=null&&corp.personname!='NULL' }">${corp.personname }</c:if></td>
						<td align="center"><wd:dicvalue dicId="1041" dicCode="${corp.corpstatus }"/></td>
						<td align="center"><c:if test="${corp.establishdate!=null }"><fmt:formatDate value="${corp.establishdate }" pattern="yyyy年MM月dd日"/></c:if></td>
						<td align="center"><a href="#" onclick="view('${corp.corpinfoid}')">查看</a></td>
					</tr>
				</c:forEach>
				<c:if test="${obj.warn != null}">
				<tr>
				    <td colspan="7" style="color:red;text-align:center">${obj.warn }!</td>
				</tr>
				</c:if>
			</table>
		</div>
		<div>
			<table width="96%" class="tables">
				<tr>
					<td>
					<c:if test="${obj.list!=null &&obj.flag==1}">
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input
					type="button" class="minButton" value="列表导出（200条）" style="width:180px" onclick="exportExecl()" /></td>
					</c:if>
					<c:if test="${obj.list!=null &&obj.flag!=1}">
					<p>&nbsp;&nbsp;&nbsp;&nbsp;您已超过当日下载上限！不能进行下载操作。</p>
					</c:if>
	...
	<form id="export" action="${ctx}/query/exportCorpList" method="post">
		...
		<input type="hidden"  name="jyfw" value="${obj.jyfw }" />
		<input type="hidden"  name="jycs" value="${obj.jycs }" />
...
<script type="text/javascript">
	...
	function query(type) {
		if($('#dwmc').val()==''&&$('#frzt').val()==''&&$('#frlx').val()==''&& $('#jyfw').val()==''&&$('#jycs').val()==''
	...
	function changedate1(){
		var date=$('#checkedDate1').val();
		$('#checkedDate2').val(date);
	}
	function changedate2(){
		var date=$("#checkedDate1").val();
		var date1=$("#checkedDate2").val();
