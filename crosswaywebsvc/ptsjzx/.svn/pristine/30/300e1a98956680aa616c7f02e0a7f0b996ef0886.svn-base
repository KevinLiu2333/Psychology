		...
		<ul>
			<c:forEach items="${obj.contents }" var="contents" varStatus="row">
				<c:if test="${ row.index==0 }">
						<li><a href="#tab${row.index+1}" class="selected">${ contents.zwm}</a></li>
				</c:if>
				<c:if test="${ row.index!=0 }">
						<li><a href="#tab${row.index+1}" >${ contents.zwm}</a></li>
				</c:if>
			</c:forEach>
		</ul>
		</div>
 		<c:forEach items="${obj.contents }" var="contents" varStatus="row">
 			<div id="tab${row.index+1 }" class="tabson">
	    		<iframe src="${ctx}/sjtb/toTbym?type=${contents.name}" height="650px" width="100%" frameborder="0"></iframe>
	    	</div>
 		</c:forEach>
	</div> 
</body>
