...
//打印
function printInfo(applyBatch){
	var href = "${ctx}/api/reloadAgreement?applyBatch="+applyBatch+"&isPrint=1";
...
<p style="text-align:left;font-size:20px;padding-bottom: 15px;padding-top: 10px;margin-left: 60px;"><b>API资源申请</b></p>
	...
	<table width="60%" class="table_multilist" align="center" style="color: navy;">
		<tr>
			...
			&nbsp;&nbsp;${obj.apiApply.department}单位：</br>
			&nbsp;&nbsp;由于${obj.apiApply.applyReason}原因</br>
			&nbsp;&nbsp;您已申请&nbsp;<font color="red">${obj.apiTypesChinese}。</font></br>
			...
			&nbsp;&nbsp;所提供的附件为：${obj.file}</br>
			&nbsp;&nbsp;请遵守以下协议要求：</br>
			&nbsp;&nbsp;${obj.apiTypesChinese}使用协议范本文字:</br>
	...
	<div style="margin-left: 480px">
		...
		<input type="button" id="printView"  class="button" value="打印预览" onclick="printInfo('${obj.apiApply.applyBatch}');"/>&nbsp;&nbsp;
		<c:if test="${obj.apiApply.isSubmit != '1'}">
			<input type="button" id="tijiao"  class="button" value="提交审核" onclick="tijiaoCheck('${obj.apiApply.applyBatch}');"/> 
		</c:if>
		<c:if test="${obj.apiApply.isSubmit == '1'}">
			<span><font color="red">申请已提交，请等待审核结果！</font></span>
			<!-- 
			<input type="button" id="tijiao" title="申请已提交，请等待审核结果！" class="button" value="提交审核"/> 
			 -->
		</c:if>
	</div></br>
</form>
