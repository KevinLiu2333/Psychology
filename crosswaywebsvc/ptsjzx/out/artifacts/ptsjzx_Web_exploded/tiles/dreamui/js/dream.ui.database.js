	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				//2013-10 add by 冯佳 解决动态表格删除行后序号不连续的问题
				var addButTxt = $table.attr('addButton') || "Add New";
				if (addButTxt) {
					var $addBut = $('<br/><table width="200px"  align="center"><tr><td><div class="buttonActive"><div class="buttonContent"><button type="button">'+addButTxt+'</button></div></div></td></tr></table>').insertAfter($table).find("button");
					//$('<span>&nbsp;&nbsp;&nbsp;&nbsp;每次增加</span>').insertBefore($table);
					var $rowNum = $('<input type="hidden" name="dwz_rowNum" class="textInput" style="margin:2px;" value="1" size="2"/>').insertBefore($table);
					//$('<span>行</span>').insertBefore($table);
					var trTm = "";
					//初始化时如果没有记录,则添加一行
					//if ($rowNum.val() == '1') {
						//addRow(trTm,fields,$tbody,$rowNum);
					//}
			 ...
			 * 增加一行
			 */
			function addRow(trTm,fields,$tbody,$rowNum){
			...
			function initSuffix($tbody) {
				$tbody.find('>tr').each(function(i){
					$(':input, a.btnLook, a.btnAttach, .customIndex, .customIndexValue', this).each(function(){
						...
						if ($this.attr('class')=="customIndex")  {$this.html(i+1);}
						
						if ($this.attr('id')=="customIndexValue")  {$this.val(i+1);}
					});
				});
			}
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					case 'date':
						...
						break;
					case 'customIndex':
						html = '<span class="customIndex"></span><input id="customIndexValue" type="hidden"  name="'+field.name+'" />';
						break;
					default:
