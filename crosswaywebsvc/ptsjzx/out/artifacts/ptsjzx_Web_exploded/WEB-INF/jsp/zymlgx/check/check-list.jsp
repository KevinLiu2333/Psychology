...
<div align="center">
	...
	<table width="96%" height="60" align="center" class="query_search" style="border:1px solid #B6CDF7;border-bottom:1px solid #B6CDF7;">
		<tr>
			...
			<td>审核状态：
				<wd:select id="stStatus" name="filter_str_status_eq" dicCode="<%=Constants.STATUS_RESOURCE_APPLY%>" initValue="---请选择---" defaultValue="${filter.filter_str_status_eq}" className="selectInput" style="width:150px;height:25px;" />
			</td>
...
<div align="center">
	...
	<table class="table_list" style="width:96%;height:98%" layoutH="115">
		...
		<tr align="center">
			...
			<td>
				<wd:dicvalue dicId="<%=Constants.DIC_DEPT_NAME %>" dicCode="${resource.userId}"/>
			</td>
			...
			<td>
				<c:if test="${(resource.status eq '10932' && obj.user.type eq '1') || (resource.status eq '10935' && obj.user.type eq'2')}">
				<a href="#" onclick="auditApply('${resource.applyId}')">审核</a>
				</c:if>
				<a href="#" onclick="queryApply('${resource.applyId}')">查看</a>
				<%-- <c:if test="${resource.status eq '10936'}">
