...
<body>
	<form id="queryForm" name="queryForm" action="${ctx }/querySGS/toCorpCfList">
	...
	<table width="96%" border="0" align="center" cellpadding="0" cellspacing="0"  class="tablelist">
		...
		<c:forEach items="${obj.list}" var="corpcf" varStatus="row">
					<tr>
						...
						<td align="center">
							<c:if test="${fn:length(corpcf.cfsy )>20}">${fn:substring(corpcf.cfsy, 0, 19)}...</c:if>
							<c:if test="${fn:length(corpcf.cfsy )<=20}">${corpcf.cfsy }</c:if>
						</td>
						<td align="center">
							<fmt:formatDate value="${corpcf.cfjdrq }" pattern="yyyy年MM月dd日"/>
						</td>
						<td align="center"><a href="#" onclick="view('${corpcf.id }')">查看</a></td>
					</tr>
		</c:forEach>
	</table>
	<div>
...
<script type="text/javascript">
function view(id){
	window.showModalDialog("${ctx}/querySGS/toCorpCfView?id="+id,self,"dialogWidth=910px;dialogHeight=600px;status:no;");
}
</script>
</html>