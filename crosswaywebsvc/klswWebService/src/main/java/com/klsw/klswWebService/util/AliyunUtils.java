...
public class AliyunUtils {
    ...
    public static String getPullStreamPath(String streamName){
        long timestamp = (new Date().getTime() + LiveConstants.EFFECTIVE_TIME) / 1000;
        String pullStreamPath = LiveConstants.CREATE_STREAM_PATH.replace("alivemovie", "Live")
                .replace("courses", streamName).replace("timestamp", String.valueOf(timestamp))
                .replace("privateKey", LiveConstants.PRIVATE_KEY);
        pullStreamPath = LiveConstants.PUBLIC_PULL_STREAM_PATH.replace("alivemovie", "Live").replace("courses", streamName) + timestamp + "-0-0-"
