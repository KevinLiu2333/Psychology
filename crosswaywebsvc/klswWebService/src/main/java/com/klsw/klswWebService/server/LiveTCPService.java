...
import com.klsw.klswWebService.server.constants.Attributes;
import com.klsw.klswWebService.service.TbCwkService;
import com.klsw.klswWebService.service.TbLiveRoomService;
import com.klsw.klswWebService.service.VisitorService;
...
import static com.klsw.klswWebService.server.constants.Types.login;
 ...
 */
@Service
public class LiveTCPService {

    private final static Logger LOGGER = LoggerFactory.getLogger(LiveTCPService.class);
    @Resource
    ...
    void execute(ChannelHandlerContext ctx, TCPMessage message) {
        switch (message.type()) {
            //登录
            case "login":
                login(ctx, message);
     ...
     * @param message 信息
     */
    private void startLiving(ChannelHandlerContext ctx, TCPMessage message) {
        try {
     ...
     * @param message 信息
     */
    private void joinLiving(ChannelHandlerContext ctx, TCPMessage message) {
        try {
            //直播间id
            Integer liveRoomId = Integer.parseInt(message.info("liveroomid"));
            //用户类型
     ...
     * @param message 信息
     */
    private void startinteracting(ChannelHandlerContext ctx, TCPMessage message) {
        try {
            UserInfo userInfo = getUserInfo(ctx);
     ...
     * @param message 信息
     */
    private void shareopern(ChannelHandlerContext ctx, TCPMessage message) {
        UserInfo userInfo = getUserInfo(ctx);
        Integer liveroomId = userInfo.getLiveRoomId();
    ...
    private void paint(ChannelHandlerContext ctx, TCPMessage message) {
        UserInfo userInfo = getUserInfo(ctx);
        Integer liveroomId = userInfo.getLiveRoomId();
        if (liveroomId == null || !userInfo.getAudtype().equals("anchor") || !liveRoomGroup.containsKey(liveroomId)) {
            return;
    ...
    static UserInfo getUserInfo(ChannelHandlerContext ctx) {
    ...
    private void setUserInfo(ChannelHandlerContext ctx, UserInfo userInfo) {
        ctx.attr(AttributeKey.valueOf(Attributes.userInfo.name())).set(userInfo);
    }
     ...
     * @param message 信息
     */
    void res(ChannelHandlerContext ctx, String type, String status, String message) {
    ...
    private void res(ChannelGroup ctx, TCPMessage msg) {
    ...
    private void res(Channel channel, String type, Map<String, String> params) {
        String response = JsonUtils.encode(TCPMessage.ResInfo("response", type, params)) + System.getProperty("line.separator");
        //返回结果
        channel.writeAndFlush(Unpooled.copiedBuffer(response.getBytes()));
    }

    private void res(ChannelHandlerContext ctx, String type, Map<String, String> params) {
        String response = JsonUtils.encode(TCPMessage.ResInfo("response", type, params)) + System.getProperty("line.separator");
        //返回结果
        ctx.writeAndFlush(Unpooled.copiedBuffer(response.getBytes()));
    }

    private void setAttributes(ChannelHandlerContext ctx, Map<String, Object> attributes) {
    ...
    boolean verify(ChannelHandlerContext ctx, TCPMessage message) {
